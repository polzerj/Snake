...
namespace Snake.Cli
{
    class Program
    {
        ...
        private static void DrawSettings(int auswahl)
        {
            ...
            {
                WriteToConsole(0, 0,
                    "Dies ist ein Snake Spiel, welches mit den Tasten 'W' für hinauf, 'A' für rechts,'S' für hinunter und 'D' für links verwendet. Alternativ sind die Pfeiltasten möglich.");
                Console.WriteLine();
            }
            else
            {
        ...
        static void DrawSpielfeld() //Zeichne Spielfeld
        {
            if (gameOver == false)
            {
                WriteToConsole(0, 0, "Fuer das Menue und Hilfe Escape druecken!");
                int a = spielfeld;
                char boarder = (Char)9608;
                boarder = '#';
                for (int i = 1; i <= spielfeld; i++)
                {
                    WriteToConsole(i, 1, "" + boarder);
                    WriteToConsole(0, a, "" + boarder);
                    WriteToConsole(a, spielfeld, "" + boarder);
                    WriteToConsole(spielfeld, i, "" + boarder);
                    a--;
        ...
        static void SetSnake() //Zeichne die Schlange
        {
            for (int i = 0; i < posix.Count; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    if (school == false) Console.Write('U'); //('\u263a');
        ...
        public static void Save()
        {
            FileStream file;
            /*FileStream*/
            file = new FileStream(Path.Join(Directory.GetCurrentDirectory(), _path), FileMode.Create);
            //file = File.Create(Directory.GetCurrentDirectory() + "//wanderXSettings.Xml"); //Erstellt in dem aktuellen verzeichnis ...

            System.Xml.Serialization.XmlSerializer writer =
                new System.Xml.Serialization.XmlSerializer(typeof(Einstellungen));
        ...
        public static void Load()
        {
            try
            {
                FileStream file = new FileStream(Path.Join(Directory.GetCurrentDirectory(), _path), FileMode.Open);
